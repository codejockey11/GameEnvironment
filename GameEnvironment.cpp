...
void WriteBrush(CBrush* brush)
{
	for (int32_t i = 0; i < brush->m_sideCount; i++)
	{
	...
	for (int32_t i = 0; i < m_materialsCount; i++)
	{
		...
		{
			m_heapMap[i] = new CHeap(32768);
			
			m_heapCollision[i] = new CHeap(32768);
		}
	}
	...
	for (int32_t i = 0; i < m_materialsCount; i++)
	{
		if (brush->m_totalVertices[i] != 0)
		{
			for (int32_t s = 0; s < brush->m_sideCount; s++)
			{
				...
				printf("m %i=====================================================\n", brush->m_brushSide[s].m_shaderMaterial->m_number);
				...
				for (int32_t v = 0; v < brush->m_brushSide[s].m_vertexCount; v++)
				{
					if (v == brush->m_brushSide[s].m_vertexCount - 1)
					{
						...
						m_heapMap[i]->Append(0);
						m_heapMap[i]->Append(v + 1);
						m_heapMap[i]->Append(1);

						break;
					}

					printf("0 %i %i ", v + 1, v + 2);

					m_heapMap[i]->Append(0);
					m_heapMap[i]->Append(v + 1);
					m_heapMap[i]->Append(v + 2);
				}

				printf("v %i\n", brush->m_brushSide[s].m_vertexCount + 1);

				m_heapMap[i]->Append(brush->m_brushSide[s].m_vertexCount + 1);

				m_heapMap[i]->Append((void*)&brush->m_brushSide[s].m_center.m_v, sizeof(CVertexNT));

				for (int32_t v = 0; v < brush->m_brushSide[s].m_vertexCount; v++)
				{
					printf("%i\n", v);

					m_heapMap[i]->Append((void*)&brush->m_brushSide[s].m_vertex[v].m_v, sizeof(CVertexNT));
				}
				...
				if (brush->m_brushSide[s].m_shaderMaterial->m_number == i)
				{
					for (int32_t v = 0; v < brush->m_brushSide[s].m_vertexCount; v++)
					{
						...
						m_heapCollision[i]->Append((unsigned char*)&brush->m_brushSide[s].m_shaderMaterial->m_surface, sizeof(int32_t));
	...
	for (int32_t i = 0; i < m_materialsCount; i++)
	{
		...
		if (m_heapCollision[i])
		{
			...
			int32_t count = m_heapCollision[i]->m_length / sizeof(CCollisionPrimitive);
...
void CreateTerrain(CEntity* entity)
{
	...
	if (m_heightmapImage)
	{
		...
		for (int32_t h = 0; h < m_terrainDepth; h++)
		{
			m_xIncrement = (float)(m_terrainWidth / 2) * -m_primSize;

			for (int32_t w = 0; w < m_terrainWidth; w++)
			{
#ifdef _DEBUG
				...
				vertex->m_p.y = (float)(m_heightmapImage->m_paletteEntries[*m_pixelStart].peRed * m_terrainHeight) + m_origin.m_p.y;
	...
	*/
	...
	for (int32_t h = 0; h < m_terrainDepth - 1; h++)
	{
		for (int32_t w = 0; w < m_terrainWidth - 1; w++)
		{
#ifdef _DEBUG
			printf("H:%03iW:%03i\r", h + 1, w + 1);
#endif
	...
	for (int32_t h = 0; h < m_terrainDepth - 1; h++)
	{
		for (int32_t w = 0; w < m_terrainWidth - 1; w++)
		{
			...
			printf("H:%03iW:%03i\r", h + 1, w + 1);
#endif
			int32_t w1 = ((w - 1) < 0) ? 0 : w - 1;
			int32_t h1 = ((h - 1) < 0) ? 0 : h - 1;

			int32_t w1h1 = m_tiles->GetOffset(2, w1, h1);

			fseek(master, w1h1, SEEK_SET);
			fread(&m_terrainTile1, sizeof(CTerrainTile), 1, master);

			int32_t w2 = ((w - 1) < 0) ? 0 : w - 1;
			int32_t h2 = h;

			int32_t w2h2 = m_tiles->GetOffset(2, w2, h2);
			...
			int32_t w3h3 = m_tiles->GetOffset(2, w3, h3);

			fseek(master, w3h3, SEEK_SET);
			fread(&m_terrainTile3, sizeof(CTerrainTile), 1, master);

			int32_t w4 = w;
			int32_t h4 = ((h - 1) < 0) ? 0 : h - 1;

			int32_t w4h4 = m_tiles->GetOffset(2, w4, h4);
	...
	for (int32_t h = 0; h < m_terrainDepth - 1; h++)
	{
		for (int32_t w = 0; w < m_terrainWidth - 1; w++)
		{
			...
			int32_t wh = m_tiles->GetOffset(2, w, h);
	...
	for (int32_t w = 0; w < m_terrainWidth - 1; w++)
	{
		...
		int32_t wh = m_tiles->GetOffset(2, w, m_terrainDepth - 2);
	...
	for (int32_t h = 0; h < m_terrainDepth - 1; h++)
	{
		for (int32_t w = 0; w < m_terrainWidth - 1; w++)
		{
			...
			printf("H:%03iW:%03i\r", h + 1, w + 1);
#endif
			int32_t w1h1 = m_tiles->GetOffset(2, w, h);
