...
void WriteCollisionPrimitiveToSectors(CCollisionPrimitive* collisionPrimitive)
{
	...
	float sy = (float)(m_sector->m_height / 2) - m_sector->m_height;
	float sz = (float)(m_sector->m_depth / 2) - m_sector->m_depth;
	...
	for (UINT y = 0; y < m_sector->m_gridHeight; y++)
	{
		for (UINT z = 0; z < m_sector->m_gridDepth; z++)
		{
...
void CreateTerrain(CEntity* entity)
{
	...
	if (m_heightmapImage)
	{
		...
		for (int h = 0; h < m_terrainDepth; h++)
		{
			m_xIncrement = (float)(m_terrainWidth / 2) * -m_primSize;
	...
	*/
	...
	for (int h = 0; h < m_terrainDepth - 1; h++)
	{
	...
	for (int w = 0; w < m_terrainWidth - 1; w++)
	{
		...
		int wh = m_tiles->GetOffset(2, w, m_terrainDepth - 2);
